...
int main() {
    ...
    ConstantChooser i12345{"12345"}, i3498{"3498"}, i700000{"700000"},i777("777");
	  ConstantGuesser n23{"23"}, n201{"201"}, n5{"5"},n13567("13567");
		...
		.CHECK_OUTPUT(calculateBullAndPgia("90","20"), "1,0")      // 1 bull, 10 pgia
		.CHECK_OUTPUT(calculateBullAndPgia("111","1093"), "1,2")   // change to error or something
		.CHECK_OUTPUT(calculateBullAndPgia("777","877"), "2,0")    // 2 bull, 0 pgia
		.CHECK_OUTPUT(calculateBullAndPgia("548","238"), "1,0")    // 1 bull, 0 pgia
		.CHECK_OUTPUT(calculateBullAndPgia("3212","54"), "0,4")    // change to error or something
    //*******************************************************//

			.CHECK_OUTPUT(calculateBullAndPgia("1000000","100000000"), "7,0")      // change to error or something
		...
		.CHECK_EQUAL(play(s202020,d202020,6, 10), 1)    //guesser wins in one turn. 
		...
		.CHECK_EQUAL(play(s, d000000, 0, 2), 3)     // guesser loses technically by choosing an illegal number (too long).
		 ...
		 .CHECK_EQUAL(play(e5641, d2222, 4, 100), 101)      // guesseloses by rr unning out of turns.
	...
	.CHECK_EQUAL(play(j13567,n13567, 5, 100), 1)      // guesser wins in one turn.
		...
		.CHECK_EQUAL(play(s0909090, m30000, 7, 100), 1)      // lose tec
